...
import com.example.managespending.data.mappers.BudgetMapper;
...
import com.example.managespending.data.remotes.services.BudgetService;
...
public class BudgetServiceImpl extends BaseService<BaseDTO> implements BudgetService {
    ...
    @Override
    public ResponseDTO<BaseDTO> getByExpiredStatus(BaseDTO baseDTO) {
        try {
            if (((BudgetDTO) baseDTO).getBudgetExpired() == null) {
                return ResponseDTO.<BaseDTO>builder()
                        .message("Please enter the budget expire status !!!")
